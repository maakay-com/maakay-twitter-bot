...
def check_confirmation():
    ...
    for txs in waiting_confirmations_txs:
        ...
        try:
            main_bank_confirmation = requests.get(f"http://{os.environ['BANK_IP']}/confirmation_blocks?block=&block__signature={txs.signature}&format=json").json()
            keysign_bank_confirmation = requests.get(f"http://{os.environ['KEYSIGN_BANK_IP']}/confirmation_blocks?block=&block__signature={txs.signature}&format=json").json()
...
def scan_chain():
    ...
    TNBC_TRANSACTION_SCAN_URL = f"http://{os.environ['BANK_IP']}/bank_transactions?account_number={os.environ['BOT_ACCOUNT_NUMBER']}&block__sender=&fee=&recipient="
    ...
    while next_url:
        ...
        for transaction in r['results']:
            ...
            if scan_tracker.last_scanned < transaction_time and not transaction_exists:
                ...
                if transaction['recipient'] == os.environ['BOT_ACCOUNT_NUMBER']:
