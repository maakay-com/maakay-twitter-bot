...
def withdraw_tnbc(recipient, amount, memo):
    ...
    signing_key = nacl.signing.SigningKey(str.encode(os.environ['SIGNING_KEY']), encoder=nacl.encoding.HexEncoder)
    ...
    try:
        r = requests.request("POST", f"http://{os.environ['BANK_IP']}/blocks", headers=headers, data=data)
    except Exception as e:
        return False, e
